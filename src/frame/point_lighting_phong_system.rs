...
pub struct PointLightingPhongSystem {
    gfx_queue: Arc<Queue>,
...
impl PointLightingPhongSystem {
    /// Initializes the point lighting system.
    pub fn new(gfx_queue: Arc<Queue>, subpass: Subpass) -> PointLightingPhongSystem {
...
void main() {
    ...
    if (lambertian > 0.0) {
        vec3 view_dir = normalize(world.xyz);
        
        vec3 half_dir = normalize(light_direction + view_dir);
        float spec_angle = max(dot(half_dir, in_normal), 0.0);
        specular = pow(spec_angle, 1.0);
    ...
    vec3 final_color = in_diffuse * lambertian / distance + specular/ distance;


    //final_color = pow(final_color, vec3(1.0 / 2.2));

    f_color.rgb = final_color;
    f_color.a = 1.0;
}",
        types_meta: {
            use bytemuck::{Pod, Zeroable};
